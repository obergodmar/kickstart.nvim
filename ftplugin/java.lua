...
if require('helpers.utils').is_mac() then
  OS_NAME = 'mac'
elseif require('helpers.utils').is_win() then
  OS_NAME = 'win'
else
  OS_NAME = 'linux'
end
...
local config = {
  cmd = {
    '/Library/Java/JavaVirtualMachines/temurin-8.jdk/Contents/Home/bin/java', -- or '/path/to/java17_or_newer/bin/java'
  ...
  capabilities = require('helpers.lsp.common').capabilities,
